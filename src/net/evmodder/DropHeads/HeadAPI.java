...
//		String localList = FileIO.loadFile("head-textures.txt", hardcodedList);  // This does not preserve comments
	...
	public String getTextureKey(PlayerProfile profile){
		...
		if(properties != null && properties.containsKey(DH_TEXTURE_KEY)){
			final Collection<Property> props = properties.get(DH_TEXTURE_KEY);
			if(props != null && !props.isEmpty()){
				if(props.size() != 1) pl.getLogger().warning("Multiple texture keys on a single head profile in getTextureKey()");
				return trimTextureKeyUntilFoundOrNull(props.iterator().next().value());
			}
		}
		...
		if(ASSIGN_KEY_FROM_TEXTURE && properties.containsKey("textures")){
			final Collection<Property> textures = properties.get("textures");
			if(textures != null && textures.size() == 1){
				final String code = textures.iterator().next().value();
				// Will return the associated textureKey if `code` is a known texture value
				return replaceHeadsFromTo.get(code);
			}
		}
		if(ASSIGN_KEY_FROM_NAMED_UUID && profile.getUniqueId() != null){
			final String key = replaceHeadsFromTo.get(profile.getUniqueId().toString());
...
//		if(code == null || code.isEmpty()) return null;
		...
		Multimap<String, Property> properties = MiscUtils.getProperties(profile);
		properties.put("textures", new Property("textures", code));
		properties.put(DH_TEXTURE_KEY, new Property(DH_TEXTURE_KEY, textureKey));
		if(MAKE_UNSTACKABLE) properties.put(DH_RANDOM_UUID, new Property(DH_RANDOM_UUID, UUID.randomUUID().toString()));
...
//		GameProfile profile = new GameProfile(uuid, name);
	...
	private ItemStack hdb_getItemHead_wrapper(String hdbId){// Calling hdbAPI.getItemHead(id) directly is bad.
		...
		PlayerProfile profile = ((SkullMeta)hdbHead.getItemMeta()).getOwnerProfile();
...
//		pl.getLogger().info("url code: "+urlCode);
	...
	public ItemStack getHead(byte[] code){
		...
		PlayerProfile profile = Bukkit.createPlayerProfile(UUID.nameUUIDFromBytes(code), /*name=*/strCode.substring(0, Math.min(strCode.length(), MAX_NAME_LENGTH)));
		Multimap<String, Property> properties = MiscUtils.getProperties(profile);
		properties.put("textures", new Property("textures", strCode));
		if(MAKE_UNSTACKABLE) properties.put(DH_RANDOM_UUID, new Property(DH_RANDOM_UUID, UUID.randomUUID().toString()));
		if(SAVE_TYPE_IN_LORE){
			head = MiscUtils.setLore(head, new RawTextComponent(
	...
	public ItemStack getHead(PlayerProfile profile/*, boolean saveTypeInLore, boolean unstackable*/){
		...
		if(MAKE_UNSTACKABLE){
			if(!properties.containsKey(DH_RANDOM_UUID)){
				properties.put(DH_RANDOM_UUID, new Property(DH_RANDOM_UUID, UUID.randomUUID().toString()));
			}
		}
		...
		//-------------------- Handle players
		final boolean updateSkin = !properties.containsKey("textures") || !LOCK_PLAYER_SKINS;
		final boolean hasProfileID = profile.getUniqueId() != null;
		final boolean hasPlayedBefore, realGameProfile;
		if(!hasProfileID) hasPlayedBefore = realGameProfile = false;
		else{
			hasPlayedBefore = pl.getServer().getOfflinePlayer(profile.getUniqueId()).hasPlayedBefore();
			final String profileName;
			final PlayerProfile freshProfile = MiscUtils.getPlayerProfile(profile.getUniqueId().toString(), updateSkin);
			if(freshProfile == null){profileName = profile.getName(); realGameProfile = false;}
				...
				if(LOCK_PLAYER_SKINS){
					...
					if(textures == null || textures.isEmpty() || textures.size() > 1){
						...
						final String minCode0 = minimizeTextureCode(textures.iterator().next().value());
						//TODO: Decide which properties to clear and which to keep
						profile = Bukkit.createPlayerProfile(profile.getUniqueId(), profile.getName());
						properties = MiscUtils.getProperties(profile);
						properties.put("textures", new Property("textures", minCode0));
		...
		//-------------------- Handle raw textures
		if(/*!hasPlayedBefore && !realGameProfile && */properties.containsKey("textures")){
			final Collection<Property> textures = properties.get("textures");
			if(textures != null && !textures.isEmpty()){
				if(textures.size() > 1) pl.getLogger().warning("Multiple textures in getHead() request: "+profile.getName());
				final String code0 = textures.iterator().next().value();
				// Confirm (as best we can) that this is a DropHeads Custom Texture Head
				if(UUID.nameUUIDFromBytes(code0.getBytes()).equals(profile.getUniqueId()) || properties.containsKey(MiscUtils.DH_LORE_KEY)){
		...
		/*if(DEBUG_MODE)*/pl.getLogger().fine("Unrecognized head profile [name:"+profile.getName()+",id:"+profile.getUniqueId()+"]");
		return head;
	}
	...
	public ItemStack getHead(Entity entity/*, boolean saveTypeInLore, boolean unstackable*/){
		if(entity.getType() == EntityType.PLAYER){
			return getHead(Bukkit.createPlayerProfile(entity.getUniqueId(), entity.getName()));
		}
